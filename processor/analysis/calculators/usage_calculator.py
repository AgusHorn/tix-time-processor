...
class UsageCalculator:
    def __init__(self, observations, clock_fixer):
        ...
        self.downstream_histogram = FixedSizeBinHistogram(observations, self.downstream_time_key_function, 0.5)
        self.upstream_usage, self.downstream_usage = self._calculate_usage()

    def _calculate_usage(self):
        """
        Calculates the usage for the upstream and the downstream

        Returns
        -------
        upstream and downstream usage as floats
        """
